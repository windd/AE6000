...
{
    Modbus_Get_TXdata(ReceiveData + 1, SendData + 1);
    if(SendData[2] == 0)return 0;
    SendData[0] = SYS[0];
    CRC16(&SendData[0], 3 + SendData[2], &SendData[3 + SendData[2]], &SendData[4 + SendData[2]]);
    return SendData[2] + 5;
}
