...
void Load_Net_Parameters(void)
{
    ...
    Sub_Mask[0] = (SYS[SYS_YM+1]&0xFF00)>>8; //加载子网掩码
    Sub_Mask[1] = SYS[SYS_YM+1]&0xFF;
    Sub_Mask[2] = (SYS[SYS_YM+2]&0xFF00)>>8;
    Sub_Mask[3] = SYS[SYS_YM+2]&0xFF;
    ...
    IP_Addr[0] = (SYS[SYS_IP]&0xFF00)>>8; //加载本机IP地址
    IP_Addr[1] = SYS[SYS_IP]&0xFF;
    IP_Addr[2] = (SYS[SYS_IP+1]&0xFF00)>>8;
    IP_Addr[3] = SYS[SYS_IP+1]&0xFF;

    S0_Port[0] = 0x1;//加载端口0的端口号502
    S0_Port[1] = 0xf6;
...
{
    unsigned short len;
    len = Read_SOCK_Data_Buffer(s, Rx_Buffer);
    len = TCP_Get_TXdata();
    Write_SOCK_Data_Buffer(s, Tx_Buffer, len);
}
void Change_IP(void)
{
